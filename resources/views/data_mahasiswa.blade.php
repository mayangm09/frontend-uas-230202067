    ...
    <div class="flex-1 flex flex-col">
      ...
      <main class="p-6">
        ...
        <h2 class="text-2xl font-semibold text-gray-700">👩‍🎓Data Mahasiswa</h2>
          ...
          <a href="..." target="_blank"class="bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded-md text-sm transition">🖨️ Cetak PDF</a>
          </div>
          ...
          <table class="min-w-full border border-gray-300 text-sm rounded-md overflow-hidden">
            <thead>
              <tr class="bg-gray-100 text-center text-gray-600">
                <th class="px-5 py-3 border-b border-gray-300">NPM</th>
                <th class="px-5 py-3 border-b border-gray-300">Nama Mahasiswa</th>
                <th class="px-5 py-3 border-b border-gray-300">Nama Kelas</th>
            ...
            <tbody id="mahasiswaTableBody"> 
              ...
              <tr class="hover:bg-gray-50 text-center">
                <td class="px-5 py-3 border-b border-gray-200">{{ $mhsw['npm']}}</td>
                <td class="px-5 py-3 border-b border-gray-200">{{ $mhsw['nama_mahasiswa']}}</td>
                <td class="px-5 py-3 border-b border-gray-200">{{ $mhsw['nama_kelas']}}</td>
                <td class="px-5 py-3 border-b border-gray-200">{{ $mhsw['nama_prodi']}}</td>
                <td class="px-5 py-3 border-b border-gray-200 flex justify-center space-x-3">
                  <!-- Tombol Edit -->
                  <a href="..." class="bg-yellow-500 text-white px-2 py-1 rounded">✏️Edit</a>
                  <!-- Tombol Hapus -->
                  <form action="{{ route('mahasiswa.destroy', $mhsw['npm']) }}" method="POST" style="display:inline;">
                  @csrf
                  @method('DELETE')
